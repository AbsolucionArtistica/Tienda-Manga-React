...
const CarritoLateral = () => {
  ...
  const finalizarCompra = () => {
    ...
    // cerrar el carrito y navegar a checkout
    cerrarCarrito()
    navigate('/checkout')
  }
      ...
      {isOpen && (
        ...
        >
          ...
          <div className="d-flex justify-content-between align-items-center p-3 border-bottom">
            ...
            <button className="btn btn-outline-secondary btn-sm" onClick={toggleCarrito}>
                ...
                {carrito.map(item => (
                  <div key={item.id} className="d-flex align-items-center mb-3 p-2 border rounded">
                    <img
                      ...
                      onError={e => { e.target.src = 'https://via.placeholder.com/50x60/cccccc/666666?text=Manga' }}
                    />
                    <div className="flex-grow-1 ms-3">
                      <h6 className="mb-1 small">{item.nombre}</h6>
                      <p className="mb-1 text-muted small">{formatearPrecio(item.precio)}</p>
                      <div className="d-flex align-items-center">
                        <button className="btn btn-outline-secondary btn-sm" style={{ width: '30px', height: '30px' }} onClick={() => actualizarCantidad(item.id, item.cantidad - 1)}>-</button>
                        <span className="mx-2 small">{item.cantidad}</span>
                        <button className="btn btn-outline-secondary btn-sm" style={{ width: '30px', height: '30px' }} onClick={() => actualizarCantidad(item.id, item.cantidad + 1)}>+</button>
                        <button className="btn btn-outline-danger btn-sm ms-2" onClick={() => eliminarDelCarrito(item.id)}>
          ...
          <div className="border-top p-3 bg-light">
            <div className="d-flex justify-content-between align-items-center mb-3">
              ...
              <span className="h5 mb-0 text-danger fw-bold">{formatearPrecio(precioTotal)}</span>
            </div>
            <div className="d-grid gap-2">
              <button className="btn btn-danger" onClick={finalizarCompra}>
